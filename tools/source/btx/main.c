...
int main(int argc, char **argv)
{
    ...
    if (argc == 1) { // only the exe name was provided
        ...
        if (dumpFlag) {
            ret = dumpBtxToPngAndMappings();
        } else {
            ret = buildBtxFromPngAndMappings();
        }
    }
    return ret;
}
...
static void writeField(FILE *file, int offset, unsigned int data, int size)
{
    ...
    if (offset == 0xB4) {
        debug_printf("A quick write to the wrong place\n");
    }
