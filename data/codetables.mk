...
SPECIESBST_TARGET := $(BUILD)/a028/9_19
SPECIESBST_DEPENDENCIES := data/SpeciesBSTTable.c
SPECIESBST_OBJS := $(patsubst data/%.c,build/%.o,$(SPECIESBST_DEPENDENCIES))
SPECIESBST_BIN := $(patsubst data/%.c,build/%.bin,$(SPECIESBST_DEPENDENCIES))

$(SPECIESBST_BIN): $(SPECIESBST_DEPENDENCIES)
	$(CC) $(CFLAGS) -c $< -o $(SPECIESBST_OBJS)
	$(OBJCOPY) -O binary $(SPECIESBST_OBJS) $@

NARC_FILES += $(SPECIESBST_BIN)

SPECIESTYPES_TARGET := $(BUILD)/a028/9_20
SPECIESTYPES_DEPENDENCIES := data/SpeciesTypesTable.c
SPECIESTYPES_OBJS := $(patsubst data/%.c,build/%.o,$(SPECIESTYPES_DEPENDENCIES))
SPECIESTYPES_BIN := $(patsubst data/%.c,build/%.bin,$(SPECIESTYPES_DEPENDENCIES))

$(SPECIESTYPES_BIN): $(SPECIESTYPES_DEPENDENCIES)
	$(CC) $(CFLAGS) -c $< -o $(SPECIESTYPES_OBJS)
	$(OBJCOPY) -O binary $(SPECIESTYPES_OBJS) $@

NARC_FILES += $(SPECIESTYPES_BIN)